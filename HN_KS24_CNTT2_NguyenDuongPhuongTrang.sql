...
-- Tên khách và số điện thoại của tất cả khách hàng
SELECT guest_name, phone FROM guests;
-- Các loại phòng khác nhau trong khách sạn
SELECT room_type FROM rooms
GROUP BY room_type;
-- Loại phòng và giá thuê theo ngày, sắp xếp theo giá tăng dần
SELECT room_type, price_per_day FROM rooms
GROUP BY room_type, price_per_day
ORDER BY price_per_day ASC ;
-- Các phòng có giá thuê lớn hơn 1.000.000
SELECT room_type, price_per_day FROM rooms
WHERE price_per_day > 1000000 ;
-- Các lần đặt phòng diễn ra trong năm 2024
SELECT guest_id, room_id, check_in, check_out FROM bookings
WHERE check_in >= '2024-01-01' AND check_out <= '2024-12-31';
-- Số lượng phòng của từng loại phòng
SELECT r.room_type, count(r.room_type) FROM rooms r
...
SELECT g.guest_name, count(r.room_type)
FROM bookings b
JOIN guests g ON g.guest_id = b.guest_id
JOIN rooms r ON r.room_id = b.room_id
GROUP BY g.guest_name;
-- Doanh thu của mỗi phòng
SELECT r.room_id, sum(r.price_per_day * (b.check_out - b.check_in)) as revenue
FROM bookings b
JOIN rooms r ON r.room_id = b.room_id
GROUP BY r.room_id;
-- Tổng doanh thu của từng loại phòng
SELECT r.room_type, sum(r.price_per_day * (b.check_out - b.check_in)) as revenue
FROM rooms r
